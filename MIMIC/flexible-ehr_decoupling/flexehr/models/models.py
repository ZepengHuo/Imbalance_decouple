...
from .lstms import LSTM, GRU


MODELS = ['Mortality', 'LOS', 'LOS7', 'pheno']


def init_model(model_type, n_tokens, latent_dim, hidden_dim, 
               coef_pos, bal_loss_r, p_dropout=0.0, dt=1.0, weighted=True, dynamic=True):
    """
    ...
    model = Model(decoder, n_tokens, latent_dim, hidden_dim, 
                  coef_pos, bal_loss_r, p_dropout=p_dropout, dt=dt, weighted=weighted,
                  dynamic=dynamic)
...
class Model(nn.Module):
    def __init__(self, decoder, n_tokens, latent_dim, hidden_dim, 
        ...
        self.gru = GRU(self.latent_dim, self.hidden_dim)
        self.decoder = decoder(self.hidden_dim)

    def forward(self, input, if_main=False):
        """
        ...
        all_hidden, final_hidden = self.gru(emb)
        if self.dynamic:
            output = self.decoder(self.dropout(all_hidden), if_main)
        else:
            output = self.decoder(self.dropout(final_hidden), if_main)
