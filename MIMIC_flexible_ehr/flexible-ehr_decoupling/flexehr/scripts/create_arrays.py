...
def create_arrays(root, token2index, t_hours=48, n_bins=20, max_len=10000):
    ...
    train_files = pd.read_csv(os.path.join(root, 'numpy', f'0-train.csv'))['Paths']
    ...
    #for patient in tqdm(os.listdir(root)):
        ...
        for i, ts_file in enumerate(patient_ts_files):
            ...
            if np.isnan(ts).any():
                #print(np.where(ts[f'TOKEN_{n_bins}'] != ts[f'TOKEN_{n_bins}']))
                print(f)
                print(ts)
                exit()
...
if __name__ == '__main__':
    ...
    t2i = np.load(
        ...
        os.path.join(args.root, 'numpy', '48-0-20.npy'),
